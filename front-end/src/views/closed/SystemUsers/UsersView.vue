        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Full_name</th><th class="px-6 py-3 text-left">Email</th><th class="px-6 py-3 text-left">Role_id</th><th class="px-6 py-3 text-left">Is_active</th><th class="px-6 py-3 text-left">Last_login</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.full_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.email }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.role_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.is_active }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.last_login }}</td>
      ...
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Full_name:</span>
              {{ item.full_name }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Role_id:</span>
              {{ item.role_id }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Last_login:</span>
              {{ item.last_login }}
            </div>
        </div>
      </div>
