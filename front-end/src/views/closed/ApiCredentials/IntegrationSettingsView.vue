...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading integrationSettings..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">IntegrationSettings</h1>
      <button @click="openAddModal" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm">
        ...
        <span>Add IntegrationSettings</span>
      </button>
    </div>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Provider_name</th><th class="px-6 py-3 text-left">Merchant_id</th><th class="px-6 py-3 text-left">Profile_id</th><th class="px-6 py-3 text-left">Access_key</th><th class="px-6 py-3 text-left">Secret_key</th><th class="px-6 py-3 text-left">Shared_secret_key</th><th class="px-6 py-3 text-left">Environment</th><th class="px-6 py-3 text-left">Is_active</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.provider_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.merchant_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.profile_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.access_key }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.secret_key }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.shared_secret_key }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.environment }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.is_active }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">IntegrationSettings #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Provider_name:</span>
              {{ item.provider_name }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Merchant_id:</span>
              {{ item.merchant_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Profile_id:</span>
              {{ item.profile_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Access_key:</span>
              {{ item.access_key }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Secret_key:</span>
              {{ item.secret_key }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Shared_secret_key:</span>
              {{ item.shared_secret_key }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Is_active:</span>
              {{ item.is_active }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-integrationsettings v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-integrationsettings v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete IntegrationSettings"
      message="Are you sure you want to delete this integrationSettings?"
...
<script>
import AddIntegrationSettings from "./AddIntegrationSettings.vue";
import EditIntegrationSettings from "./EditIntegrationSettings.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddIntegrationSettings, EditIntegrationSettings, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/integrationsettings', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'IntegrationSettings-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/integrationsettings', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('IntegrationSettings deleted successfully', 'success');
      }
