...
class StreamlinedBillingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<StreamlinedBillingScreen> createState() => _StreamlinedBillingScreenState();
}

class _StreamlinedBillingScreenState extends ConsumerState<StreamlinedBillingScreen> {
  ...
  void _populateFieldsFromUser(User user) {
    ...
    setState(() {
      ...
      if (user.kyc != null) {
        ...
        _administrativeAreaController.text = user.kyc!.city; // Use city as state for simplicity
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryRow('Amount', '${widget.amount.toStringAsFixed(2)} ${widget.currency}'),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  '\$${(widget.amount / widget.exchangeRate).toStringAsFixed(2)}',
 ...
 Widget _buildBillingInfoSection() {
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 12),
              Text(
                _useLoginInfo ? 'Confirm Billing Details' : 'Enter Billing Details',
                style: const TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                  color: Colors.black87,
              ...
              Expanded(
                child: TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    fillColor: (_useLoginInfo || _isLoadingUserData) ? Colors.grey.shade100 : null,
                  ),
          ...
          DropdownButtonFormField<String>(
            ...
            onChanged: (_useLoginInfo || _isLoadingUserData) ? null : (value) {
              if (value != null) {
                setState(() {
        ...
        child: _isLoadingUserData
            ...
            : Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 12),
                  Text(
                    'Pay \$${(widget.amount / widget.exchangeRate).toStringAsFixed(2)} Now',
                    style: const TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                    ),
  ...
  void _proceedToPayment() {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      ref.read(paymentFormProvider.notifier).updateAccountHolder(widget.toAccountHolder);
      ...
      ref.read(paymentFormProvider.notifier).updateExchangeRate(widget.exchangeRate);
      
      // Update billing info
      ref.read(paymentFormProvider.notifier).updateFirstName(_firstNameController.text.trim());
      ref.read(paymentFormProvider.notifier).updateLastName(_lastNameController.text.trim());
      ref.read(paymentFormProvider.notifier).updateEmail(_emailController.text.trim());
      ref.read(paymentFormProvider.notifier).updateAddress1(_address1Controller.text.trim());
      ref.read(paymentFormProvider.notifier).updateLocality(_localityController.text.trim());
      ref.read(paymentFormProvider.notifier).updateAdministrativeArea(_administrativeAreaController.text.trim());
      ref.read(paymentFormProvider.notifier).updatePostalCode(_postalCodeController.text.trim());
      ref.read(paymentFormProvider.notifier).updateCountry(_selectedCountry);
      ref.read(paymentFormProvider.notifier).updateRemark(_remarkController.text.trim());
